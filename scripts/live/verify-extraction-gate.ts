...
async function main() {
  ...
  const source = b.contexts()[0].pages().find((x) => x.url().includes("5003"));
  if (!source) throw new Error("manager OpenChat page is unavailable");
  const tabs = new TemporaryTabScope();
  const failuresBefore = failures;
  let artifactScope: OpenChatArtifactScope | undefined;
  let primaryFailed = false;
  try {
  const p = await tabs.open(source, "http://localhost:5003/chats", {
    manualExtract: true,
  });
  ...
  // Issue 1 test seam: opt into deterministic extraction only in this disposable tab.
  await armManualExtractForCurrentUrl(p);

  const cards = () => p.locator(".action-card").count();
  const nonce = Date.now() % 100000;
  const rejectedText = `gate check ${nonce}: hello there`;
  const positiveText = `gate check pos ${nonce}: cleaning fee 350 EGP`;
  const positiveNote = `gatecheck-pos ${nonce}`;
  artifactScope = new OpenChatArtifactScope(p, "extraction-gate live proof");
  await artifactScope.begin();
  artifactScope.expectExactText(rejectedText);
  artifactScope.expectExactText(positiveText);
  artifactScope.expectCardInputs([positiveNote]);
  ...
  if (failures > 0) {
    throw new Error(`GATE VERIFY FAILED — ${failures} assertion(s)`);
  }
