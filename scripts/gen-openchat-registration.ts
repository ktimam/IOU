...
// (promptTemplate, responseSchema, rules, public card rows, perUserKeys) are copied here so a human can paste them into
...
// sends. Card chrome remains paste-only, while the public row list is generated from the same TS
// source the runtime registrar uses.
const regenerated = {
  ...doc,
...
for (const k of ["promptTemplate", "responseSchema", "card", "perUserKeys", "rules"] as const) {
