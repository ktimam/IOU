...
function token(sub: string, key = oc.privateKey, ttlMs = 5 * 60 * 1000): string {
  const now = Date.now();
  return signOpenChatToken({ sub, iat: now, exp: now + ttlMs }, key);
}
...
async function main() {
  const srv = spawn("./node_modules/.bin/tsx", ["scripts/iou-relay/server.ts"], {
    env: { ...process.env, IOU_RELAY_PORT: String(PORT), IOU_OPENCHAT_PUBKEY: PUBKEY_PEM },
  ...
  try {
    ...
    const started = await (await post("/v1/pairings/start", { token: TOKEN })).json();
    ...
    const forged = await post("/v1/pairings/claim", { code: started.code, oc_token: token("oc_alice", evil.privateKey) });
    ok(forged.status === 401, "claim with FORGED provenance token → 401");

    // 3) OpenChat claims the pairing with a valid token
    const claim = await post("/v1/pairings/claim", { code: started.code, oc_token: token("oc_alice") });
    ...
    const reuse = await post("/v1/pairings/claim", { code: started.code, oc_token: token("oc_alice") });
    ok(reuse.status === 404, "pairing code is single-use (404 on reuse)");

    // 5) IOU Settings lists the pairing
    const list = await (await fetch(`${BASE}/v1/pairings?token=${TOKEN}`)).json();
    ...
    const fwd = await post("/v1/openchat/drafts", { oc_token: token("oc_alice"), draft: DRAFT });
    ...
    const unpaired = await post("/v1/openchat/drafts", { oc_token: token("oc_bob"), draft: DRAFT });
    ok(unpaired.status === 409, "openchat/drafts for unpaired user → 409 (no route)");

    // 9) an EXPIRED provenance token is rejected
    const expired = await post("/v1/openchat/drafts", { oc_token: token("oc_alice", oc.privateKey, -1000), draft: DRAFT });
    ok(expired.status === 401, "expired provenance token → 401");

    // 10) revoke unbinds the user; further drafts no longer route
    const del = await fetch(`${BASE}/v1/pairings/oc_alice?token=${TOKEN}`, { method: "DELETE" });
    ok((await del.json()).removed === 1, "DELETE /v1/pairings/oc_alice → removed");
    const afterRevoke = await post("/v1/openchat/drafts", { oc_token: token("oc_alice"), draft: DRAFT });
