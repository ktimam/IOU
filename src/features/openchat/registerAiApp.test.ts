...
describe("buildManifestWire", () => {
  ...
  it("opts the IOU action into app-authorized account-scoped recipients", () => {
    ...
    expect(manifest.actions[0]?.recipient_scope).toEqual([
      { app_authorized: null },
    ]);
  });
  ...
  it("sends inbox_canister_id as an opt principal when supplied, [] otherwise", () => {
    ...
    // A non-principal (e.g. the local dfx alias) is tolerated and omitted, not thrown.
    const aliasInbox = buildManifestWire(
      "",
      undefined,
      () => {},
      "iou_backend",
  ...
  it("does not leak supplied account templates into the public wire manifest", () => {
    ...
    expect(
      action.card.rows.find((row) => row.field === "template"),
    ).toBeUndefined();
  ...
  it("omits template metadata when the regression seam receives an empty list", () => {
    const manifest = buildManifestWire(
      ...
      actions: {
        rules: { keyword_map?: { field: string } }[];
        response_schema: string;
      }[];
    };
    const action = manifest.actions[0];
    expect(
      action.rules.find((r) => r.keyword_map?.field === "template"),
    ).toBeUndefined();
    expect(
      JSON.parse(action.response_schema).properties.template,
    ).toBeUndefined();
  ...
  it("matches OpenChat's verifier-v2 canonical manifest commitment golden", () => {
    const principal = (byte: number) =>
      Principal.fromUint8Array(Uint8Array.of(byte));
    const manifest = {
      name: "Sample App",
    ...
    expect({ digest, encodedHex }).toEqual({
      digest:
        "93f82f31d31c7cea66bd8d6e434b77442c035d7386f52fad4019250625db71b9",
      encodedHex:
  ...
  it("carries only raw-free registered surfaces with snake-label display variants", () => {
    ...
    expect(
      surfaces.every((s) => !/[?&](?:chat|message|user)=/i.test(s.url)),
    ).toBe(true);
