...
const SCHEMA = {
    ...
    "x-iou-source-grounded-transactions": {
        ...
        sourceField: "message",
        fallbackKind: "iou",
        requireOcrEvidenceFields: undefined as unknown,
...
describe("source-grounded transaction schema opt-in", () => {
    it("accepts only a complete, bounded declaration", () => {
        ...
        expect(
            supportsSourceGroundedTransactions({
                ...SCHEMA,
                "x-iou-source-grounded-transactions": {
                    ...SCHEMA["x-iou-source-grounded-transactions"],
                    surprise: true,
                },
            }),
        ).toBe(false);
    ...
    it("accepts only an app-declared OCR default that belongs to the direction enum", () => {
        ...
        invalid["x-iou-source-grounded-transactions"].ocrDefaultDirection = "sideways";
    ...
    it("accepts only the exact bounded model-first browser image strategy", () => {
        ...
        for (const replacement of [
            ...
            if (replacement === undefined) {
                delete candidate["x-iou-browser-image-strategy"];
            } else {
                candidate["x-iou-browser-image-strategy"] = replacement;
            }
        ...
        delete noFallbackParser["x-iou-source-grounded-transactions"];
...
describe("typed source parsing", () => {
    ...
    it("keeps anchored sequences available and requires an explicit whole-message opt-in", () => {
        ...
        delete (anchorOnly["x-iou-text-sequence"] as Record<string, unknown>).unanchoredMode;
        delete (anchorOnly["x-iou-text-sequence"] as Record<string, unknown>).unanchoredLabels;
        expect(
    ...
    it("deduplicates repeated keyword declarations and bounds relationship-hit work", () => {
        ...
        (directionRule!.map[0] as unknown as { keywords: string[] }).keywords = Array.from({ length: 64 }, () => "owe me");
    ...
    it("passes the ordinary, type/date, delimited and natural multi-entry acceptance inputs", () => {
        ...
        const datedService = parseText(
            "workshop 3-8 august 7777 gbp",
            new Date("2026-08-14T09:00:00Z"),
        );
        expect(datedService).toEqual({
            kind: "candidates",
            candidates: [
                {
                    amount: 7777,
                    ...
                    direction: "debt",
                    date: "2026-08-03",
                    note: "workshop 3-8 august",
                    message: "workshop 3-8 august 7777 gbp",
                },
            ],
        });
    ...
    it("keeps a generic duration in the note while using its start as the date", () => {
        expect(
            parseText("Workshop 1-10 August 700 USD", new Date("2026-08-16T09:00:00Z")),
        ).toEqual({
            kind: "candidates",
            candidates: [
                {
                    ...
                    direction: "debt",
                    date: "2026-08-01",
                    note: "Workshop 1-10 August",
                    message: "Workshop 1-10 August 700 USD",
                },
            ],
        });
    });
    ...
    it("does not truncate the exact source field or exceed the configured cardinality", () => {
        ...
        constrained["x-iou-source-grounded-transactions"].maximumItems = 1;
        expect(
...
describe("OCR source parsing", () => {
    it("uses separate Arabic OCR only as semantic evidence for English money", () => {
        ...
        evidenceRequired["x-iou-source-grounded-transactions"].requireOcrEvidenceFields = [
            "kind",
        ];
        expect(
    ...
    it("accepts an app-labelled success amount, exact OCR currency alias, and note", () => {
        const transcript = [
            ...
            "Note Bill Payments - TEST-UNIT-A1",
            "POWERED BY",
        ].join("\n");
        expect(parseOcr(transcript)).toEqual({
            kind: "candidates",
    ...
    it("uses the app-declared OCR default only when relationship evidence is absent", () => {
        ...
        delete (noOcrDefault["x-iou-source-grounded-transactions"] as Record<string, unknown>)
            .ocrDefaultDirection;
        expect(
    ...
    it("can require explicit OCR evidence for a mapped semantic field without changing text defaults", () => {
        ...
        for (const invalid of [
            ...
            candidate["x-iou-source-grounded-transactions"].requireOcrEvidenceFields = invalid;
