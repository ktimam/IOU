...
describe("IOU private matcher isolation", () => {
  ...
  it("binds one parent WindowProxy/origin/attempt and returns no metadata", () => {
    ...
    expect(page).not.toContain("finish(false)");
  ...
  it("checks the authoritative exact-source commitment before roster decryption", () => {
    expect(context.indexOf("exactPrivateMatchSource(exactMessageText")).toBeGreaterThan(-1);
    expect(context.indexOf("exactPrivateMatchSource(exactMessageText")).toBeLessThan(
      context.indexOf("sheetKey = await unwrapSheetKeyProd("),
    );
  ...
  it("verifies the full exact source before deriving persisted matcher evidence", () => {
    expect(page.indexOf("loadPrivateMatchContext(request.capability, session, exactMessageText)")).toBeLessThan(
