import { parseDeclaredTextSequence } from "./localExtractionSemantics";

// A deliberately narrow, deterministic extractor for apps that opt into the
// `x-iou-source-grounded-transactions` response-schema extension. It consumes either the
...
export const SOURCE_GROUNDED_TRANSACTIONS_EXTENSION = "x-iou-source-grounded-transactions";
export const BROWSER_IMAGE_STRATEGY_EXTENSION = "x-iou-browser-image-strategy";
...
/** Whether the opted-in parser can make progress from its separately bounded semantic reader.
 * Keep parser reason strings private to this module so host orchestration has no field semantics. */
export function sourceGroundedParseNeedsSemanticEvidence(
    result: SourceGroundedParseResult,
): boolean {
    return (
        (result.kind === "none" && result.reason === "missing_transaction_semantics") ||
        (result.kind === "ambiguous" && result.reason === "missing_ocr_direction")
    );
}
...
function declaredParser(schema: object | undefined): Declaration | undefined {
    ...
    return {
        ...
        dateEnabled:
            ...
            dateProperty["x-iou-normalize-date"] === true &&
            dateProperty["x-iou-date-from-text"] === true,
    };
}
...
function mappedValue(
    ...
    // A longer declared phrase owns a nested shorter keyword (`priority alert` beats `alert`). Sort
    // once and sweep the two strongest differently-valued covering spans; this keeps
...
function identifierNumberSpans(text: string): Span[] {
    ...
    // Any short source label can introduce an explicit identifier. The parser must not carry a
    // roster of document categories merely to keep those digits out of the amount candidates.
    const prefixedIdentifier =
        /\b[\p{L}][\p{L}\p{M}'’.-]*(?:\s+[\p{L}][\p{L}\p{M}'’.-]*){0,2}\s+(?:id|number|no\.?)\s*#?\s*(\d{2,})\b/giu;
    const genericId = /\bid\s*(?:number|no\.?|#|:)?\s*#?\s*(\d{2,})\b/giu;
    return [
        ...capturedNumberSpans(text, labelled),
        ...capturedNumberSpans(text, prefixedIdentifier),
...
function extractTypedNote(
    ...
    // A range is both the normalized date source and meaningful transaction duration. Keep its
    ...
    for (let index = authorityIndex - 1; index >= 0; index--) {
        ...
        if (relation !== undefined) {
            ...
            if (
                ...
                // source's only descriptive category or heading, so preserve that exact visible
                // text instead of discarding it after classification.
                continue;
            }
        }
        const dates = scanDates(line);
 ...
 * I/O and is suitable for IOU's private extraction surface.
 */
 ...
 * is an app-owned copy rather than a reference to its configuration object.
 */
